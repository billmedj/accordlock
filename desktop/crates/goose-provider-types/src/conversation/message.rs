    ...
    pub fn from_provider_error(err: &crate::errors::ProviderError) -> Self {
        ...
        let text = match err {
            ProviderError::NotConfigured => {
                "No model is connected.\n\nConnect a model in Settings, then try again."
            }
            ProviderError::Authentication(_) => {
                "Your model connection needs attention.\n\nReconnect the provider in Settings, then try again."
            }
            ProviderError::ContextLengthExceeded(_) => {
                "This task is too large for the model's context window.\n\nStart a new task or choose a model with a larger context window."
            }
            ProviderError::RateLimitExceeded { .. } => {
                "The model provider is temporarily busy.\n\nWait a moment, then try again."
            }
            ProviderError::ServerError(_) => {
                "The model provider is temporarily unavailable.\n\nTry again in a moment."
            }
            ProviderError::NetworkError(_) => {
                "AccordLock could not reach the model provider.\n\nCheck your connection, then try again."
            }
            ProviderError::RequestFailed(_) => {
                "The model provider could not process this request.\n\nTry again. If it continues, check the provider connection in Settings."
            }
            ProviderError::ExecutionError(_) | ProviderError::UsageError(_) => {
                "The model provider could not complete this request.\n\nTry again. If it continues, switch models or check the provider connection in Settings."
            }
            ProviderError::NotImplemented(_) => {
                "This model does not support a capability required for this task.\n\nChoose another model in Settings, then try again."
            }
            ProviderError::EndpointNotFound(_) => {
                "The configured model endpoint could not be found.\n\nCheck the provider connection in Settings, then try again."
            }
            ProviderError::CreditsExhausted { .. } => {
                "This provider account has no available credits.\n\nAdd credits with the provider, then try again."
            }
            ProviderError::Refusal { .. } => {
                "The model provider declined this request.\n\nStart a new task and revise the request before trying again."
            }
        };
...
mod tests {
    ...
    #[test]
    fn provider_error_messages_do_not_expose_internal_details() {
        use std::time::Duration;

        const PRIVATE_DETAILS: &str =
            "https://private-provider.example/v1/chat?api_key=secret diagnostic payload";
        let cases = [
            ProviderError::NotConfigured,
            ProviderError::Authentication(PRIVATE_DETAILS.to_string()),
            ProviderError::ContextLengthExceeded(PRIVATE_DETAILS.to_string()),
            ProviderError::RateLimitExceeded {
                details: PRIVATE_DETAILS.to_string(),
                retry_delay: Some(Duration::from_secs(30)),
            },
            ProviderError::ServerError(PRIVATE_DETAILS.to_string()),
            ProviderError::NetworkError(PRIVATE_DETAILS.to_string()),
            ProviderError::RequestFailed(PRIVATE_DETAILS.to_string()),
            ProviderError::ExecutionError(PRIVATE_DETAILS.to_string()),
            ProviderError::UsageError(PRIVATE_DETAILS.to_string()),
            ProviderError::NotImplemented(PRIVATE_DETAILS.to_string()),
            ProviderError::EndpointNotFound(PRIVATE_DETAILS.to_string()),
            ProviderError::CreditsExhausted {
                details: PRIVATE_DETAILS.to_string(),
                top_up_url: Some(PRIVATE_DETAILS.to_string()),
            },
            ProviderError::Refusal {
                details: PRIVATE_DETAILS.to_string(),
                category: Some(PRIVATE_DETAILS.to_string()),
            },
        ];

        for provider_error in cases {
            let message = Message::from_provider_error(&provider_error);
            let visible_error = message
                .content
                .iter()
                .find_map(MessageContentBlock::as_error)
                .expect("provider errors should be presented as typed error content");

            assert!(
                !visible_error.message.contains(PRIVATE_DETAILS),
                "user-visible error leaked internal details for {provider_error:?}"
            );
            assert!(
                !visible_error.message.contains("private-provider.example"),
                "user-visible error leaked the provider endpoint for {provider_error:?}"
            );
            assert!(!visible_error.message.trim().is_empty());
            assert_eq!(visible_error.kind, MessageErrorKind::from(&provider_error));
        }
    }

    #[test]
    fn provider_error_messages_give_actionable_recovery_guidance() {
        let authentication = Message::from_provider_error(&ProviderError::Authentication(
            "invalid token".to_string(),
        ));
        let network = Message::from_provider_error(&ProviderError::NetworkError(
            "connection refused".to_string(),
        ));
        let context = Message::from_provider_error(&ProviderError::ContextLengthExceeded(
            "maximum tokens exceeded".to_string(),
        ));

        fn visible_error(message: &Message) -> &str {
            message
                .content
                .iter()
                .find_map(MessageContentBlock::as_error)
        ...
        assert!(visible_error(&authentication).contains("Reconnect"));
        assert!(visible_error(&network).contains("Check your connection"));
        assert!(visible_error(&context).contains("Start a new task"));
    }
