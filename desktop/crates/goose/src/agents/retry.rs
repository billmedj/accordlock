...
impl RetryManager {
    ...
    pub async fn handle_retry_logic(
        ...
        retry_config.validate().map_err(anyhow::Error::msg)?;
...
pub async fn execute_shell_command(
    ...
    Err(anyhow::anyhow!(
        "recipe shell execution is disabled in the AccordLock distribution"
    ))
}
...
mod tests {
    ...
    #[cfg(feature = "accordlock-distribution")]
    #[tokio::test]
    async fn accordlock_execution_guard_rejects_retry_and_on_failure_shells() {
        let check = SuccessCheck::Shell {
            command: "echo bypass".to_string(),
        };
        let timeout = Duration::from_secs(1);

        let direct = execute_shell_command("echo bypass", timeout).await;
        let success_check = execute_success_checks_with_timeout(&[check], timeout).await;
        let on_failure = execute_on_failure_command_with_timeout("echo bypass", timeout).await;

        for result in [direct.map(|_| ()), success_check.map(|_| ()), on_failure] {
            assert!(
                result.is_err_and(|error| error.to_string().contains("disabled in the AccordLock"))
            );
        }
    }
