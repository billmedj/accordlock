    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        #[cfg(feature = "accordlock-distribution")]
        {
            if !self.checks.is_empty() {
                return Err(
                    "shell success checks are disabled in the AccordLock distribution".to_string(),
                );
            }
            if self.on_failure.is_some() {
                return Err(
                    "shell on_failure commands are disabled in the AccordLock distribution"
                        .to_string(),
                );
            }
        }
...
mod tests {
    ...
    #[cfg(feature = "accordlock-distribution")]
    #[test]
    fn accordlock_rejects_every_recipe_shell_escape() {
        let shell_check = RetryConfig {
            max_retries: 1,
        ...
        assert_eq!(
            shell_check.validate(),
            Err("shell success checks are disabled in the AccordLock distribution".to_string())
        );

        let on_failure = RetryConfig {
            max_retries: 1,
            checks: Vec::new(),
            on_failure: Some("echo bypass".to_string()),
            timeout_seconds: None,
            on_failure_timeout_seconds: None,
        };
        assert_eq!(
            on_failure.validate(),
            Err(
                "shell on_failure commands are disabled in the AccordLock distribution".to_string()
            )
        );
    }
}
