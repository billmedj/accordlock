...
use crate::agents::tool_execution::{
    capture_agent_plan_checkpoint, tool_stream, ToolCallResult, ToolStreamItem,
...
impl<'a> ToolExecutionOperation<'a> {
    ...
    async fn dispatch_tool_call(
        ...
        session: &Session,
        agent_plan_checkpoint: crate::agents::accordlock_authorization::AgentPlanCheckpointInput,
        ...
        async {
            ...
            .with_agent_plan_checkpoint(agent_plan_checkpoint);
            let result = self
                .extension_manager
...
impl Operation<Session, GooseEffect> for ToolExecutionOperation<'_> {
    ...
    async fn run(
        ...
        for (request, disposition) in &pending {
            ...
            let assistant_message = messages_since_kickoff(conversation)?
                .iter()
                .rev()
                .find(|message| {
                    message.role == Role::Assistant
                        && message.content.iter().any(|content| {
                            matches!(content, MessageContent::ToolRequest(candidate) if candidate.id == request.id)
                        })
                })
                .ok_or_else(|| anyhow!("missing assistant source message for tool call"))?;
            let agent_plan_checkpoint =
                capture_agent_plan_checkpoint(assistant_message, &session.id, &request.id)
                    .map_err(|error| anyhow!("invalid agent plan checkpoint: {error}"))?;
            let result = self
                .dispatch_tool_call(
                    tool_call,
                    request.id.clone(),
