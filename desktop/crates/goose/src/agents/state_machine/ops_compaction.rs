...
pub(super) const COMPACTION_FAILURE_MESSAGE: &str =
    "AccordLock could not shorten this task's history.\n\n\
    ...
    async fn run_command(
        ...
        .await
        {
            ...
            Err(error) => {
                ...
                tracing::error!("Compaction failed: {error}");
                return Self::command_error(
                    conversation,
                    COMPACTION_FAILURE_MESSAGE.to_string(),
                    emit,
                )
                .await;
            }
    ...
    async fn run(
        ...
        .await
        {
            ...
            Err(e) => {
                ...
                emit.message(Message::assistant().with_text(COMPACTION_FAILURE_MESSAGE))
                    .await;
                yielded()
            }
        }
