...
impl Operation<Session, GooseEffect> for RetryOperation<'_> {
    ...
    async fn run(
        ...
        *self.grind.lock().await = None;
        ...
        if let Err(error) = validate_retry_config_for_execution(retry_config) {
            let message = emit.message(retry_error(&error)).await;
            return applied([message.into()]);
        }
...
fn validate_retry_config_for_execution(retry_config: &RetryConfig) -> Result<(), String> {
    retry_config.validate()
}

#[cfg(all(test, feature = "accordlock-distribution"))]
mod accordlock_tests {
    use super::*;
    use crate::agents::types::SuccessCheck;

    #[test]
    fn accordlock_state_machine_rejects_recipe_shell_before_retry_execution() {
        let retry = RetryConfig {
            max_retries: 1,
            checks: vec![SuccessCheck::Shell {
                command: "echo bypass".to_owned(),
            }],
            on_failure: Some("echo cleanup".to_owned()),
            timeout_seconds: None,
            on_failure_timeout_seconds: None,
        };

        assert!(validate_retry_config_for_execution(&retry).is_err());
    }
}
