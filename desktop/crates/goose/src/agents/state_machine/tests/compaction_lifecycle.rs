...
use crate::agents::state_machine::ops_compaction::{
    COMPACTION_FAILURE_MESSAGE, MAX_CONTEXT_ERROR_COMPACTIONS,
};
...
async fn a_failed_compact_command_reports_the_error_and_keeps_working() -> Result<()> {
    ...
    const PRIVATE_DETAILS: &str =
        "summarizer offline at https://private-provider.example/v1?token=secret";
    api.on(SUMMARIZE_HISTORY).server_error(PRIVATE_DETAILS);
    pipeline.run(["do some work"]).await?;

    let failed = pipeline.run(["/compact"]).await?;
    assert_eq!(failed.history_replacements(), 0);
    failed.assert_message(-1, Agent, COMPACTION_FAILURE_MESSAGE);
    assert!(failed
        .conversation()
        .messages()
        .iter()
        .all(|message| !message.as_concat_text().contains(PRIVATE_DETAILS)));
    assert!(failed
        .conversation()
        .messages()
        .iter()
...
#[tokio::test]
async fn a_failed_automatic_compaction_hides_provider_details() -> Result<()> {
    let (pipeline, api) = test_pipeline().await?;
    const PRIVATE_DETAILS: &str =
        "HTTP 500 from https://private-provider.example/v1 with payload {secret:true}";
    api.on("seed the task").reply("seeded");
    api.on(SUMMARIZE_HISTORY).server_error(PRIVATE_DETAILS);
    pipeline.run(["seed the task"]).await?;
    pipeline
        .set_total_tokens((pipeline.context_limit() as f64 * 0.81) as i32)
        .await;

    let failed = pipeline.run(["continue"]).await?;
