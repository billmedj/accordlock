...
pub(crate) fn flatpak_spawn_command() -> tokio::process::Command {
    ...
    command.args(FLATPAK_HOST_ARGS).args([
        "--unset-env=ACCORDLOCK_RUNTIME_URL",
        "--unset-env=ACCORDLOCK_RUNTIME_TOKEN",
        "--unset-env=ACCORDLOCK_BACKEND_BINDING_SECRET",
    ]);
    crate::subprocess::scrub_accordlock_authority(&mut command);
    command
}
...
fn flatpak_spawn_process() -> std::process::Command {
    ...
    crate::subprocess::scrub_accordlock_authority_std(&mut command);
    command
}
...
pub(crate) fn resolve_login_shell_path() -> Option<String> {
    ...
    let mut cmd = if is_flatpak() {
        ...
        c.args(login_args);
        crate::subprocess::scrub_accordlock_authority_std(&mut c);
        CommandWrap::from(c)
    };
