...
impl Envs {
    ...
    const DISALLOWED_KEYS: [&'static str; 34] = [
        // AccordLock authority belongs exclusively to the Goose process.
...
timeout: 300",
    ...
    #[test]
    fn extension_environments_cannot_receive_accordlock_authority() {
        let envs = extension::Envs::new(std::collections::HashMap::from([
            ("ACCORDLOCK_RUNTIME_URL".to_owned(), "runtime".to_owned()),
            ("accordlock_runtime_token".to_owned(), "token".to_owned()),
            (
                "ACCORDLOCK_BACKEND_BINDING_SECRET".to_owned(),
                "binding".to_owned(),
            ),
        ...
        assert_eq!(
            envs.get_env(),
            std::collections::HashMap::from([("SAFE_VALUE".to_owned(), "visible".to_owned())])
        );
    }
}
