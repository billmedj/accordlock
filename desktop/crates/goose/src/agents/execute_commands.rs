...
pub fn context_management_unsupported_message(command: &str, _provider: &str) -> String {
    format!(
        "/{command} is unavailable because the connected provider manages its own conversation context."
    )
}
...
impl Agent {
    ...
    async fn handle_prompt_command(
            ...
            .await
        {
            ...
            Err(e) => {
                tracing::warn!(error = %e, "Prompt command failed");
                Ok(Some(
                    Message::assistant().with_text(PROMPT_LOAD_FAILURE_MESSAGE),
                ))
            }
        }
    }
