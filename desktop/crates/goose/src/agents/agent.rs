...
use crate::agents::extension_manager::{
    enforce_accordlock_extension_policy, get_parameter_names, ExtensionManager,
...
const COMPACTION_FAILURE_MESSAGE: &str = "AccordLock could not shorten this task's history.\n\n\
Try again. If the problem continues, start a new task.";
const COMMAND_FAILURE_MESSAGE: &str =
    "AccordLock could not run that command. Check the command and try again.";
const EXTENSION_LOAD_FAILURE_MESSAGE: &str =
    "AccordLock could not start this connection. Review its settings and try again.";
const RETRY_FAILURE_MESSAGE: &str =
    "AccordLock could not continue this task. Try again or start a new task.";
const CREDITS_EXHAUSTED_MESSAGE: &str =
    "Add credits to your model provider account, then resend your message to continue.";
...
impl Agent {
    ...
    async fn handle_approved_and_denied_tools(
        ...
        for request in &permission_check_result.approved {
            if let Ok(tool_call) = request.tool_call.clone() {
                let plan_checkpoint = crate::agents::tool_execution::capture_agent_plan_checkpoint(
                    assistant_message,
                    &session.id,
                    &request.id,
                )?;
    ...
    async fn insert_frontend_extension(&self, extension: ExtensionConfig) {
        // Frontend tools execute outside the Rust dispatch boundary. Keep them
        // unavailable until the desktop implements a governed execution and
        // receipt round trip; exposing them would create a second effect path.
        warn!(
            extension = %extension.key(),
            "frontend extension disabled by AccordLock distribution"
        );
    }
    ...
    pub async fn dispatch_tool_call(
        ...
        session: &Session,
        agent_plan_checkpoint: Option<super::accordlock_authorization::AgentPlanCheckpointInput>,
        ...
        let ctx = match agent_plan_checkpoint {
            Some(checkpoint) => ctx.with_agent_plan_checkpoint(checkpoint),
            None => ctx,
        };
                        ...
                        .await
                    {
                        ...
                        Err(e) => {
                            ...
                            ExtensionLoadResult {
                                name,
                                success: false,
                                error: Some(EXTENSION_LOAD_FAILURE_MESSAGE.to_string()),
                            }
                        }
                    }
                }
    ...
    async fn add_extension_inner(
        ...
        enforce_accordlock_extension_policy(&extension, false)?;
        let session = self
            .config
            .session_manager
            .get_session(session_id, false)
    ...
    async fn reply_impl(
        ...
        match command_result {
            Err(e) => {
                error!(error = %e, "Slash command failed");
                let error_message = Message::assistant()
                    .with_text(COMMAND_FAILURE_MESSAGE)
            ...
            let final_conversation = if !needs_auto_compact {
                ...
                .await
                {
                    ...
                    Err(e) => {
                        ...
                        yield AgentEvent::Message(
                            Message::assistant().with_text(COMPACTION_FAILURE_MESSAGE)
                        );
                        return;
                    }
                }
            ...
            loop {
                ...
                loop {
                    ...
                    match next {
                        ...
                        Err(ref provider_err @ ProviderError::CreditsExhausted { .. }) => {
                            provider_errored = true;
                            #[cfg(feature = "telemetry")]
                            crate::posthog::emit_error(
                                provider_err.telemetry_type(),
                            ...
                            error!(
                                provider_error_kind = provider_err.telemetry_type(),
                    ...
                    match final_output {
                        ...
                        None => {
                            ...
                            match self.handle_retry_logic(&mut conversation, &session_config, &initial_messages).await {
                                ...
                                Err(e) => {
                                    error!(error = %e, "Retry logic failed");
                                    yield AgentEvent::Message(
                                        Message::assistant().with_text(RETRY_FAILURE_MESSAGE)
                                    );
                                    exit_chat = true;
                                }
                            }
