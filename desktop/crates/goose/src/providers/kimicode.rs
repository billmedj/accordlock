...
#[cfg(feature = "accordlock-distribution")]
pub(crate) const KIMICODE_OAUTH_TOKEN_CACHE_KEY: &str = "KIMICODE_OAUTH_TOKEN_CACHE";

#[cfg(any(not(feature = "accordlock-distribution"), test))]
fn token_cache_path() -> std::path::PathBuf {
    Paths::in_config_dir("kimicode/token.json")
}

pub(crate) fn has_configured_token() -> bool {
    TokenCache::new().has_token()
}
...
impl TokenCache {
    fn new() -> Self {
        ...
        {
            Self {
                backend: TokenCacheBackend::Vault,
            }
        }
        ...
        {
            Self {
                backend: TokenCacheBackend::File(token_cache_path()),
            }
        }
    }

    #[cfg(test)]
    fn for_path(path: std::path::PathBuf) -> Self {
        Self {
            backend: TokenCacheBackend::File(path),
        }
    }
    ...
    fn has_token(&self) -> bool {
        match &self.backend {
            #[cfg(any(not(feature = "accordlock-distribution"), test))]
            TokenCacheBackend::File(path) => std::fs::read_to_string(path)
                .ok()
                .and_then(|raw| serde_json::from_str::<KimiToken>(&raw).ok())
                .is_some(),
            #[cfg(feature = "accordlock-distribution")]
            TokenCacheBackend::Vault => {
                crate::providers::credential_vault::contains(KIMICODE_OAUTH_TOKEN_CACHE_KEY)
                    .unwrap_or(false)
            }
        }
    }

    async fn load(&self) -> Result<Option<KimiToken>> {
        match &self.backend {
            #[cfg(any(not(feature = "accordlock-distribution"), test))]
            TokenCacheBackend::File(path) => {
            ...
            #[cfg(feature = "accordlock-distribution")]
            TokenCacheBackend::Vault => {
                crate::providers::credential_vault::load(KIMICODE_OAUTH_TOKEN_CACHE_KEY)
            }
        }
    }
    ...
    async fn save(&self, token: &KimiToken) -> Result<()> {
        match &self.backend {
            ...
            TokenCacheBackend::File(path) => {
                ...
                tokio::fs::write(path, serde_json::to_string(token)?).await?;
                #[cfg(unix)]
                {
            ...
            #[cfg(feature = "accordlock-distribution")]
            TokenCacheBackend::Vault => {
                crate::providers::credential_vault::save(KIMICODE_OAUTH_TOKEN_CACHE_KEY, token)
            }
        }
    }
    ...
    async fn clear(&self) -> Result<()> {
        match &self.backend {
            #[cfg(any(not(feature = "accordlock-distribution"), test))]
            TokenCacheBackend::File(path) => match tokio::fs::remove_file(path).await {
                Ok(()) => Ok(()),
            ...
            #[cfg(feature = "accordlock-distribution")]
            TokenCacheBackend::Vault => {
                crate::providers::credential_vault::delete(KIMICODE_OAUTH_TOKEN_CACHE_KEY)
            }
        }
    }
}
...
impl KimiCodeProvider {
    ...
    async fn ensure_token(&self) -> Result<KimiToken, ProviderError> {
        ...
        if let Some(token) = self.token_cache.load().await.map_err(ProviderError::from)? {
            let usable = self.use_or_refresh(token).await?;
            *guard = Some(usable.clone());
            return Ok(usable);
        }
        ...
        loop {
            ...
            match self.do_refresh_token(&token.refresh_token).await {
                Ok(refreshed) => {
                    ...
                    if let Err(error) = self.token_cache.save(&refreshed).await {
                        #[cfg(feature = "accordlock-distribution")]
                        return Err(ProviderError::from(error));

                        #[cfg(not(feature = "accordlock-distribution"))]
                        tracing::warn!("failed to persist refreshed kimicode token: {}", error);
                    }
                    return Ok(refreshed);
                }
                Err(error) => {
                    ...
                    if !reloaded {
                        ...
                        if let Some(persisted) =
                            self.token_cache.load().await.map_err(ProviderError::from)?
                        {
                            if persisted != token {
                                token = persisted;
                                continue;
...
mod tests {
    ...
    fn test_provider(server_uri: &str, device_id: &str) -> KimiCodeProvider {
        KimiCodeProvider {
            client: Client::new(),
            token_cache: TokenCache::for_path(
                std::env::temp_dir().join(format!("goose-kimicode-test-{}.json", Uuid::new_v4())),
            ),
            cached_token: tokio::sync::Mutex::new(None),
    ...
    #[test]
    #[serial_test::serial]
    fn accordlock_default_token_cache_uses_vault_without_creating_a_file() {
        ...
        assert!(matches!(cache.backend, TokenCacheBackend::Vault));
    ...
    async fn use_or_refresh_reloads_token_rotated_by_another_provider() {
        ...
        second.token_cache = first.token_cache.clone();
        let expired = KimiToken {
