...
#[cfg(feature = "accordlock-distribution")]
pub(crate) const CHATGPT_CODEX_OAUTH_TOKEN_CACHE_KEY: &str = "CHATGPT_CODEX_OAUTH_TOKEN_CACHE";
...
impl TokenCache {
    pub(crate) fn new() -> Self {
        ...
        #[cfg(not(feature = "accordlock-distribution"))]
        {
            ...
            Self {
                backend: TokenCacheBackend::File(cache_path),
            }
        }
    }

    #[cfg(test)]
    fn for_path(cache_path: PathBuf) -> Self {
        Self {
            backend: TokenCacheBackend::File(cache_path),
        }
    }
    ...
    fn load(&self) -> Result<Option<TokenData>> {
        match &self.backend {
            #[cfg(any(not(feature = "accordlock-distribution"), test))]
            TokenCacheBackend::File(cache_path) => Ok(std::fs::read_to_string(cache_path)
                .ok()
                .and_then(|contents| serde_json::from_str(&contents).ok())),
            #[cfg(feature = "accordlock-distribution")]
            TokenCacheBackend::Vault => {
                crate::providers::credential_vault::load(CHATGPT_CODEX_OAUTH_TOKEN_CACHE_KEY)
            }
        }
    }
    ...
    pub(crate) fn has_token(&self) -> bool {
        match &self.backend {
            #[cfg(any(not(feature = "accordlock-distribution"), test))]
            TokenCacheBackend::File(_) => self.load().ok().flatten().is_some(),
            #[cfg(feature = "accordlock-distribution")]
            TokenCacheBackend::Vault => {
                crate::providers::credential_vault::contains(CHATGPT_CODEX_OAUTH_TOKEN_CACHE_KEY)
                    .unwrap_or(false)
            }
        }
    }
    ...
    fn save(&self, token_data: &TokenData) -> Result<()> {
        match &self.backend {
            #[cfg(any(not(feature = "accordlock-distribution"), test))]
            TokenCacheBackend::File(cache_path) => {
                let contents = serde_json::to_string(token_data)?;
                write_private_file(cache_path, &contents)?;
                Ok(())
            }
            #[cfg(feature = "accordlock-distribution")]
            TokenCacheBackend::Vault => crate::providers::credential_vault::save(
                CHATGPT_CODEX_OAUTH_TOKEN_CACHE_KEY,
                token_data,
            ),
        }
    }
    ...
    fn clear(&self) -> Result<()> {
        match &self.backend {
            ...
            #[cfg(feature = "accordlock-distribution")]
            TokenCacheBackend::Vault => {
                crate::providers::credential_vault::delete(CHATGPT_CODEX_OAUTH_TOKEN_CACHE_KEY)
            }
        }
    }
}
...
impl ChatGptCodexAuthProvider {
    ...
    #[cfg(not(feature = "accordlock-distribution"))]
    fn clear_cached_tokens(&self) -> Result<()> {
        self.cache.clear()
    }
...
mod tests {
    ...
    #[test]
    #[serial_test::serial]
    fn token_cache_roundtrip_uses_explicit_file_fixture() {
        let root = tempfile::tempdir().unwrap();
        let cache = TokenCache::for_path(root.path().join("tokens.json"));
        ...
        assert!(cache.has_token());
        assert_eq!(cache.load().unwrap().unwrap().access_token, "access");
    }
    ...
    fn token_cache_replaces_loose_file_with_owner_only_permissions() {
        ...
        let cache = TokenCache::for_path(cache_path.clone());
