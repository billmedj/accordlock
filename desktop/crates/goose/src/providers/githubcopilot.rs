...
pub(crate) const GITHUB_COPILOT_CACHE_VAULT_KEY: &str = "GITHUB_COPILOT_API_INFO_CACHE";
...
impl DiskCache {
    ...
    async fn load(&self) -> Result<Option<CopilotState>> {
        #[cfg(feature = "accordlock-distribution")]
        {
            let states = crate::providers::credential_vault::load::<HashMap<String, CopilotState>>(
                GITHUB_COPILOT_CACHE_VAULT_KEY,
            )?;
            Ok(states.and_then(|mut states| states.remove(&self.vault_entry_id())))
        }
    ...
    async fn save(&self, info: &CopilotState) -> Result<()> {
        #[cfg(feature = "accordlock-distribution")]
        {
            let mut states = crate::providers::credential_vault::load::<
                HashMap<String, CopilotState>,
            >(GITHUB_COPILOT_CACHE_VAULT_KEY)?
            .unwrap_or_default();
            states.insert(self.vault_entry_id(), info.clone());
            crate::providers::credential_vault::save(GITHUB_COPILOT_CACHE_VAULT_KEY, &states)
        }
        ...
        #[cfg(not(feature = "accordlock-distribution"))]
        {
            ...
            tokio::task::spawn_blocking(move || write_private_file(&cache_path, &contents))
                .await??;
            Ok(())
        }
    }
    ...
    async fn clear(&self) -> Result<()> {
        #[cfg(feature = "accordlock-distribution")]
        {
            crate::providers::credential_vault::delete(GITHUB_COPILOT_CACHE_VAULT_KEY)
        }
...
impl GithubCopilotProvider {
    ...
    async fn get_api_info(&self) -> Result<(String, String), ProviderError> {
        ...
        if let Some(state) = self.cache.load().await.map_err(ProviderError::from)? {
            if state.expires_at > Utc::now() {
                validate_copilot_api_endpoint(&state.info.endpoints.api)?;
...
mod tests {
    ...
    #[cfg(feature = "accordlock-distribution")]
    #[test]
    fn vault_cache_ids_keep_github_hosts_separate() {
        assert_eq!(
            DiskCache::new(DEFAULT_GITHUB_HOST).vault_entry_id(),
            DEFAULT_GITHUB_HOST
        );
        assert_ne!(
            DiskCache::new(DEFAULT_GITHUB_HOST).vault_entry_id(),
            DiskCache::new("github.enterprise.example").vault_entry_id()
        );
    }

    #[cfg(all(unix, not(feature = "accordlock-distribution")))]
    #[tokio::test]
