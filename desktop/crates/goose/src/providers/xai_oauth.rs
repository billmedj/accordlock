...
#[cfg(feature = "accordlock-distribution")]
pub(crate) const XAI_OAUTH_TOKEN_CACHE_KEY: &str = "XAI_OAUTH_TOKEN_CACHE";
...
impl TokenCache {
    ...
    fn load(&self) -> Result<Option<TokenData>> {
        match &self.backend {
            ...
            TokenCacheBackend::Vault => {
                crate::providers::credential_vault::load(XAI_OAUTH_TOKEN_CACHE_KEY)
            }
        }
    }
    ...
    pub(crate) fn has_token(&self) -> bool {
        match &self.backend {
            ...
            #[cfg(feature = "accordlock-distribution")]
            TokenCacheBackend::Vault => {
                crate::providers::credential_vault::contains(XAI_OAUTH_TOKEN_CACHE_KEY)
                    .unwrap_or(false)
            }
        }
    }
    ...
    fn save(&self, token_data: &TokenData) -> Result<()> {
        match &self.backend {
            ...
            #[cfg(feature = "accordlock-distribution")]
            TokenCacheBackend::Vault => {
                crate::providers::credential_vault::save(XAI_OAUTH_TOKEN_CACHE_KEY, token_data)
            }
        }
    }
    ...
    fn clear(&self) -> Result<()> {
        match &self.backend {
            ...
            #[cfg(feature = "accordlock-distribution")]
            TokenCacheBackend::Vault => {
                crate::providers::credential_vault::delete(XAI_OAUTH_TOKEN_CACHE_KEY)
            }
        }
    }
}
...
impl XaiOAuthAuthProvider {
    ...
    async fn get_valid_token(&self) -> Result<TokenData, ProviderError> {
        if let Some(mut token_data) = self.cache.load().map_err(ProviderError::from)? {
            ...
            if let Some(reloaded) = self.cache.load().map_err(ProviderError::from)? {
            ...
            match refresh_access_token(&token_data.refresh_token).await {
                ...
                Err(error @ ProviderError::Authentication(_)) => {
                    ...
                    self.cache.clear().map_err(ProviderError::from)?;
                    return Err(error);
                }
                Err(error) => {
...
mod tests {
    ...
    async fn missing_token_does_not_start_oauth() {
        ...
        let auth_provider = XaiOAuthAuthProvider {
            cache: TokenCache::for_path(directory.path().join("missing.json")),
