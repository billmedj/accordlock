...
pub async fn get_setup_catalog_entries() -> Vec<ProviderSetupCatalogEntry> {
    let mut entries = goose_providers::canonical::catalog::get_setup_catalog_entries(
        super::providers()
            .await
            .into_iter()
    ...
    entries.retain(|entry| {
        super::provider_registry::accordlock_catalog_provider_allowed(
            &entry.provider_id,
            entry.category,
            entry.acp,
        )
...
mod tests {
    ...
    #[tokio::test]
    async fn opencode_zen_is_a_native_model_provider() {
        let entry = crate::providers::get_from_registry("opencode")
            .await
        ...
        assert_eq!(metadata.default_model, "x-preview-f-free");
        assert!(metadata
            .config_keys
            .iter()
            .any(|key| key.name == "OPENCODE_API_KEY" && key.secret));

        let setup_entries = get_setup_catalog_entries().await;
        let setup_entry = setup_entries
            .iter()
            .find(|candidate| candidate.provider_id == "opencode")
            .expect("OpenCode Zen should be visible in provider setup");
        assert_eq!(setup_entry.setup_method, ProviderSetupMethod::SingleApiKey);
    }

    #[cfg(feature = "accordlock-distribution")]
    #[tokio::test]
    async fn accordlock_setup_catalog_excludes_agent_executors() {
        let entries = get_setup_catalog_entries().await;

        assert!(entries
            .iter()
            .all(|entry| entry.category != ProviderSetupCategory::Agent && !entry.acp));
        for provider_id in [
            "goose",
            "cursor-agent",
            "claude-acp",
            ...
            assert!(
                entries.iter().all(|entry| entry.provider_id != provider_id),
                "agent executor {provider_id} must not be advertised"
            );
        }
