...
pub(crate) const HUGGINGFACE_OAUTH_CACHE_VAULT_KEY: &str = "HUGGINGFACE_OAUTH_TOKEN_CACHE";
...
pub fn load_oauth_token() -> Result<Option<HuggingFaceTokenData>> {
    crate::providers::credential_vault::load(HUGGINGFACE_OAUTH_CACHE_VAULT_KEY)
}
...
#[cfg(feature = "accordlock-distribution")]
pub fn has_oauth_token() -> Result<bool> {
...
#[cfg(feature = "accordlock-distribution")]
pub fn usable_oauth_token() -> Result<Option<String>> {
    Ok(load_oauth_token()?.and_then(|token| (!token.is_expired()).then_some(token.access_token)))
}
...
#[cfg(feature = "accordlock-distribution")]
pub fn has_usable_or_refreshable_oauth_token() -> Result<bool> {
    Ok(load_oauth_token()?.is_some_and(|token| {
        !token.is_expired()
            || token
                .refresh_token
                .as_deref()
...
pub fn has_configured_token() -> Result<bool> {
    #[cfg(feature = "accordlock-distribution")]
    let has_oauth_token = has_usable_or_refreshable_oauth_token()?;
    #[cfg(not(feature = "accordlock-distribution"))]
    let has_oauth_token = has_usable_or_refreshable_oauth_token();

    has_configured_token_from_sources(has_oauth_token, hf_token_secret)
}
...
pub fn resolve_token() -> Result<Option<String>> {
    #[cfg(feature = "accordlock-distribution")]
    let oauth_token = usable_oauth_token()?;
    #[cfg(not(feature = "accordlock-distribution"))]
    let oauth_token = usable_oauth_token();

    resolve_token_from_sources(None, oauth_token, hf_token_secret)
}
...
pub async fn resolve_token_async_with_provider_token(
    ...
    #[cfg(feature = "accordlock-distribution")]
    {
        // Vault access and persistence failures are security-boundary failures,
        // not signals to silently downgrade to another credential source.
        let oauth_token =
            refreshed_or_usable_oauth_token_from_vault(oauth_client_id(), TOKEN_URL).await?;
        resolve_token_from_sources(None, oauth_token, hf_token_secret)
    }
...
pub fn clear_oauth_token() -> Result<()> {
    #[cfg(feature = "accordlock-distribution")]
    {
        crate::providers::credential_vault::delete(HUGGINGFACE_OAUTH_CACHE_VAULT_KEY)
    }
...
fn save_oauth_token(token_data: HuggingFaceTokenData) -> Result<()> {
    #[cfg(feature = "accordlock-distribution")]
    {
        crate::providers::credential_vault::save(HUGGINGFACE_OAUTH_CACHE_VAULT_KEY, &token_data)
    }
...
#[cfg(all(unix, any(not(feature = "accordlock-distribution"), test)))]
...
#[cfg(all(not(unix), any(not(feature = "accordlock-distribution"), test)))]
...
#[cfg(feature = "accordlock-distribution")]
async fn refreshed_or_usable_oauth_token_from_vault(
    client_id: &str,
    token_url: &str,
) -> Result<Option<String>> {
    let Some(token) = load_oauth_token()? else {
        return Ok(None);
    };
    ...
    let refreshed = token_data_from_response_with_refresh_fallback(refreshed, Some(refresh_token));
    let access_token = refreshed.access_token.clone();
    crate::providers::credential_vault::save(HUGGINGFACE_OAUTH_CACHE_VAULT_KEY, &refreshed)?;
