...
pub(crate) const DATABRICKS_OAUTH_CACHE_VAULT_KEY: &str = "DATABRICKS_OAUTH_TOKEN_CACHE";
...
pub fn cleanup_oauth_cache() -> Result<()> {
    ...
    {
        crate::providers::credential_vault::delete(DATABRICKS_OAUTH_CACHE_VAULT_KEY)
    }
...
impl TokenCache {
    ...
    fn load_token(&self) -> Result<Option<TokenData>> {
        #[cfg(feature = "accordlock-distribution")]
        let token_data = crate::providers::credential_vault::load::<HashMap<String, TokenData>>(
            DATABRICKS_OAUTH_CACHE_VAULT_KEY,
        )?
        .and_then(|mut tokens| tokens.remove(&self.vault_entry_id()));

        #[cfg(not(feature = "accordlock-distribution"))]
        let token_data = fs::read_to_string(&self.cache_path)
            .ok()
            .and_then(|contents| serde_json::from_str::<TokenData>(&contents).ok());

        let Some(token_data) = token_data else {
            return Ok(None);
        };

        // Only return tokens that have a refresh token. An access-only cache
        // cannot keep the Databricks session alive without user interaction.
    ...
    fn save_token(&self, token_data: &TokenData) -> Result<()> {
        ...
        {
            let mut tokens =
                crate::providers::credential_vault::load::<HashMap<String, TokenData>>(
                    DATABRICKS_OAUTH_CACHE_VAULT_KEY,
                )?
                .unwrap_or_default();
            tokens.insert(self.vault_entry_id(), token_data.clone());
            crate::providers::credential_vault::save(DATABRICKS_OAUTH_CACHE_VAULT_KEY, &tokens)
        }
...
mod tests {
    ...
    #[cfg(feature = "accordlock-distribution")]
    #[test]
    fn databricks_vault_cache_id_is_bound_to_connection_inputs() {
        ...
        assert_ne!(first.vault_entry_id(), second.vault_entry_id());
    }
