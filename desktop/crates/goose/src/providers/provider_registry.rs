...
const LEGACY_AGENT_PROVIDER_IDS: [&str; 3] = ["claude-code", "gemini-cli", "codex"];

pub(crate) fn accordlock_provider_allowed(
    ...
    #[cfg(feature = "accordlock-distribution")]
    {
        ...
        let acp = metadata.setup.as_ref().is_some_and(|setup| setup.acp);
        accordlock_catalog_provider_allowed(&metadata.name, category, acp)
    }

    #[cfg(not(feature = "accordlock-distribution"))]
    {
        let _ = (metadata, provider_type);
        true
    }
}

pub(crate) fn accordlock_catalog_provider_allowed(
    provider_id: &str,
    ...
    #[cfg(feature = "accordlock-distribution")]
    {
        !LEGACY_AGENT_PROVIDER_IDS.contains(&provider_id)
    ...
    #[cfg(not(feature = "accordlock-distribution"))]
    {
        let _ = (provider_id, category, acp);
        true
    }
}
...
impl ProviderEntry {
    fn enforce_accordlock_provider_policy(&self) -> Result<()> {
        if accordlock_provider_allowed(&self.metadata, self.provider_type) {
            return Ok(());
        }
        anyhow::bail!(
            "AccordLock does not permit agent-executing provider '{}'",
            self.metadata.name
        )
    }
    ...
    pub async fn create_with_default_model(
        ...
        self.enforce_accordlock_provider_policy()?;
...
impl ProviderRegistry {
    ...
    pub fn all_metadata_with_types(&self) -> Vec<(ProviderMetadata, ProviderType)> {
        self.entries
            .values()
            .filter(|entry| accordlock_provider_allowed(&entry.metadata, entry.provider_type))
...
mod tests {
    ...
    #[cfg(feature = "accordlock-distribution")]
    #[test]
    fn accordlock_provider_policy_rejects_agent_executors() {
        use crate::providers::catalog::{
            ProviderSetupGroup, ProviderSetupMetadata, ProviderSetupMethod,
        };

        let metadata = |name: &str, setup: Option<ProviderSetupMetadata>| {
            let mut metadata =
                ProviderMetadata::new(name, name, "test", "model", vec![], "", vec![]);
            metadata.setup = setup;
            metadata
        };

        assert!(!accordlock_provider_allowed(
            &metadata("codex", None),
            ProviderType::Builtin
        ));
        assert!(!accordlock_provider_allowed(
            &metadata(
                "agent-provider",
                Some(ProviderSetupMetadata::new(
        ...
        assert!(accordlock_provider_allowed(
            &metadata(
                "api-provider",
                Some(ProviderSetupMetadata::api_key(ProviderSetupGroup::Default)),
            ),
            ProviderType::Builtin
        ));
        assert!(accordlock_provider_allowed(
            &metadata(
                "custom-agent-shaped-provider",
                Some(ProviderSetupMetadata::cli_agent("custom", &[])),
            ),
            ProviderType::Custom
        ));
    }
    ...
    #[cfg(feature = "accordlock-distribution")]
    #[tokio::test]
    async fn accordlock_rejects_agent_provider_before_constructor() {
        let entry = crate::providers::get_from_registry("codex")
            .await
            .expect("legacy Codex provider remains registered for compatibility");
        let error = match entry.create(vec![]).await {
            Ok(_) => panic!("agent-executing provider must be rejected"),
            Err(error) => error,
        };
        assert!(error
            .to_string()
            .contains("does not permit agent-executing provider"));
    }
