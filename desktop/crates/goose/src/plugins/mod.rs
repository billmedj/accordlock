...
fn update_plugin_at_root(
    ...
    #[cfg(feature = "accordlock-distribution")]
    {
        let _ = (now, install_root, name);
        bail!("remote plugin updates are disabled in the AccordLock distribution");
    }
...
fn clone_git_repo(source: &str, destination: &Path) -> Result<()> {
    #[cfg(feature = "accordlock-distribution")]
    {
        ...
        bail!("remote plugin cloning is disabled in the AccordLock distribution");
    }
...
mod tests {
    ...
    #[cfg(feature = "accordlock-distribution")]
    #[test]
    fn accordlock_skill_discovery_cannot_auto_update_clone_or_mutate_plugins() {
        let install_root = tempfile::tempdir().unwrap();
        let plugin_dir = install_root.path().join("pinned-plugin");
        fs::create_dir_all(plugin_dir.join("skills")).unwrap();
        let metadata_path = plugin_dir.join(INSTALL_METADATA);
        let metadata = r#"{"source":"https://attacker.invalid/plugin.git","source_type":"git","format":"gemini","auto_update":true}"#;
        fs::write(&metadata_path, metadata).unwrap();
        let sentinel_path = plugin_dir.join("skills").join("sentinel.txt");
        fs::write(&sentinel_path, "unchanged").unwrap();

        let updates = auto_update_plugins_at_root(Utc::now(), install_root.path());

        assert!(updates.is_empty());
        assert_eq!(fs::read_to_string(&metadata_path).unwrap(), metadata);
        assert_eq!(fs::read_to_string(&sentinel_path).unwrap(), "unchanged");
        let clone_destination = install_root.path().join("implicit-clone");
        assert!(clone_git_repo("https://attacker.invalid/plugin.git", &clone_destination).is_err());
        assert!(!clone_destination.exists());
        assert!(update_plugin_at_root(Utc::now(), install_root.path(), "pinned-plugin").is_err());
        assert_eq!(fs::read_to_string(&metadata_path).unwrap(), metadata);
    }
