...
pub struct GooseAcpAgent {
    ...
    accordlock_working_dir: PathBuf,
}
...
#[cfg(feature = "accordlock-distribution")]
fn canonical_existing_directory(cwd: &Path) -> Result<PathBuf, agent_client_protocol::Error> {
    validate_absolute_cwd(cwd)?;
    std::fs::canonicalize(cwd)
        .map_err(|_| agent_client_protocol::Error::invalid_params().data("invalid directory path"))
}

#[cfg(feature = "accordlock-distribution")]
fn accordlock_pinned_cwd(
    trusted: &Path,
    requested: &Path,
) -> Result<PathBuf, agent_client_protocol::Error> {
    let canonical_requested = canonical_existing_directory(requested)?;
    if canonical_requested != trusted {
        return Err(agent_client_protocol::Error::invalid_params()
            .data("AccordLock sessions are pinned to the backend workspace"));
    }
    Ok(trusted.to_path_buf())
}
...
impl GooseAcpAgent {
    ...
    pub async fn new(options: GooseAcpAgentOptions) -> Result<Self> {
        #[cfg(feature = "accordlock-distribution")]
        let accordlock_working_dir = std::env::current_dir()?.canonicalize()?;
        ...
        Ok(Self {
            ...
            #[cfg(feature = "accordlock-distribution")]
            accordlock_working_dir,
        })
    }

    pub(super) fn trusted_session_cwd(
        &self,
        requested: &Path,
    ) -> Result<PathBuf, agent_client_protocol::Error> {
        validate_absolute_cwd(requested)?;

        #[cfg(feature = "accordlock-distribution")]
        {
            accordlock_pinned_cwd(&self.accordlock_working_dir, requested)
        }
    ...
    async fn apply_acp_extension_overrides(
        ...
        if let Err(error) = agent
            ...
            .await
        {
            warn!(error = %error, "AccordLock rejected the ACP developer client");
        }
    }
    ...
    async fn prepare_session_for_activation(
        ...
        let cwd = self.trusted_session_cwd(&cwd)?;
...
mod tests {
    ...
    #[cfg(feature = "accordlock-distribution")]
    #[test]
    fn accordlock_backend_workspace_cannot_be_rebound_by_acp() {
        let trusted = tempfile::tempdir().unwrap();
        let other = tempfile::tempdir().unwrap();
        let canonical_trusted = trusted.path().canonicalize().unwrap();

        assert_eq!(
            accordlock_pinned_cwd(&canonical_trusted, trusted.path()).unwrap(),
            canonical_trusted
        );
        let error = accordlock_pinned_cwd(&canonical_trusted, other.path())
            .expect_err("ACP must not move an AccordLock backend to another workspace");
        assert_eq!(
            error.data,
            Some(serde_json::json!(
                "AccordLock sessions are pinned to the backend workspace"
            ))
        );
    }
