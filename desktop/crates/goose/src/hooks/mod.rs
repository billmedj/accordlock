...
impl HookManager {
    ...
    pub async fn emit(&self, event: HookEvent, ctx: HookContext) {
        if cfg!(feature = "accordlock-distribution") {
            // Hook actions are arbitrary subprocesses outside the governed tool
            // receipt. Keep the entire hook surface unavailable until hook
            // execution itself becomes a typed, permitted effect.
            return;
        }
...
fn hook_command(command: &str, plugin_root: &Path, path: Option<&str>) -> Command {
    #[cfg(not(windows))]
    {
        if crate::agents::platform_extensions::developer::shell::is_flatpak() {
            ...
            process
                .arg("--unset-env=ACCORDLOCK_RUNTIME_URL")
                .arg("--unset-env=ACCORDLOCK_RUNTIME_TOKEN")
                .arg("--unset-env=ACCORDLOCK_BACKEND_BINDING_SECRET");
            crate::subprocess::scrub_accordlock_authority(&mut process);
