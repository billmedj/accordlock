...
mod tests {
    ...
    #[test]
    fn opencode_zen_uses_the_official_dynamic_openai_compatible_endpoint() {
        let config =
            deserialize_provider_config(crate::opencode::JSON).expect("opencode.json should parse");

        assert_eq!(config.name, "opencode");
        assert_eq!(config.engine, ProviderEngine::OpenAI);
        ...
        assert!(config
            .models
            .iter()
            .any(|model| model.name == "x-preview-f-free"));
    }
