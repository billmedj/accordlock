...
mod native_tls_tests {
    ...
    fn generated_rsa_keys() -> &'static (String, String) {
    ...
    fn generated_sec1_ec_key() -> String {
        let key = SecretKey::random(&mut OsRng);
        let generated_der = key.to_sec1_der().expect("encode SEC1 test key");
        let generated_key = sec1::EcPrivateKey::try_from(generated_der.as_slice())
            .expect("decode generated SEC1 test key");
        let key_with_curve = sec1::EcPrivateKey {
            private_key: generated_key.private_key,
            parameters: Some(sec1::EcParameters::NamedCurve(
                pkcs8::ObjectIdentifier::new_unwrap("1.2.840.10045.3.1.7"),
            )),
            public_key: generated_key.public_key,
        };
        let der = key_with_curve
            .to_der()
            .expect("encode SEC1 test key with curve");
        pem::encode(&pem::Pem::new("EC PRIVATE KEY", der))
    }

    #[test]
    fn test_pkcs8_key_passthrough() {
        let pkcs8_key = &generated_rsa_keys().0;
        let result = convert_key_to_pkcs8_pem(pkcs8_key).unwrap();
        assert_eq!(result, pkcs8_key.as_str());
    }

    #[test]
    fn test_pkcs1_rsa_key_conversion() {
        let pkcs1_key = &generated_rsa_keys().1;
        let result = convert_key_to_pkcs8_pem(pkcs1_key).unwrap();
    ...
    fn test_sec1_ec_key_conversion() {
        let sec1_key = generated_sec1_ec_key();
        let result = convert_key_to_pkcs8_pem(&sec1_key).unwrap();
        assert!(result.contains("-----BEGIN PRIVATE KEY-----"));
        assert!(result.contains("-----END PRIVATE KEY-----"));
