...
pub async fn update(canary: bool, reconfigure: bool) -> Result<()> {
    #[cfg(feature = "accordlock-distribution")]
    {
        ...
        bail!(
            "AccordLock source alpha has no approved update channel; automatic updates are unavailable. Rebuild the reviewed source and native-runtime pair instead."
        );
    }

    #[cfg(all(not(feature = "accordlock-distribution"), feature = "disable-update"))]
    {
        let _ = (canary, reconfigure);
...
mod tests {
    ...
    #[cfg(feature = "accordlock-distribution")]
    #[tokio::test]
    async fn test_accordlock_update_is_unconditionally_unavailable() {
        let error = update(false, false).await.unwrap_err();
        let message = error.to_string();
        assert!(message.contains("no approved update channel"));
        assert!(message.contains("automatic updates are unavailable"));
    }
