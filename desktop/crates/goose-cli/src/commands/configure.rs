...
fn try_store_secret(config: &Config, key_name: &str, value: String) -> anyhow::Result<bool> {
    match config.set_secret(key_name, &value) {
        ...
        Err(e) => {
            ...
            let remediation = "Unlock or repair the operating-system credential vault, or use an administrator-managed environment secret. Plaintext secret files are disabled in AccordLock";
            #[cfg(not(feature = "accordlock-distribution"))]
            let remediation = "Please ensure your system's secure storage is accessible. Alternatively you can run with GOOSE_DISABLE_KEYRING=true or set the key in your environment variables";

            cliclack::outro(
                style(format!(
                    "Failed to store {} securely: {}. {}",
                    key_name, e, remediation
                ))
                .on_red()
                .white(),
            )?;
...
pub fn configure_keyring_dialog() -> anyhow::Result<()> {
    cliclack::outro(
        "AccordLock requires the operating-system credential vault. Plaintext file storage is not available in this distribution.",
    )?;
    Ok(())
}
