...
# AccordLock source-alpha boundary: never resolve or download an upstream binary.
# A reviewed hardened binary must be mounted explicitly by the evaluator.
if [ ! -f "$GOOSE_BIN" ]; then
    echo "❌ AccordLock source alpha does not download or install agent binaries." >&2
    echo "Mount the reviewed accordlock-distribution binary at $GOOSE_BIN." >&2
    exit 64
fi
