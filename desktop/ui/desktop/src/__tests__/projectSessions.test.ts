...
describe('groupSessionsByProject', () => {
  ...
  it('uses explicit project metadata before workspace paths', () => {
    const groups = groupSessionsByProject(
      [
        makeSession({ id: 'a', projectId: 'release', workingDir: '/tmp/api' }),
        makeSession({ id: 'b', projectId: 'release', workingDir: '/tmp/web' }),
        makeSession({ id: 'c', projectId: 'research', workingDir: '/tmp/api' }),
        makeSession({ id: 'd', workingDir: '/tmp/api' }),
      ],
      [
        { id: 'release', title: 'Public release' },
        { id: 'research', title: 'Safety research' },
      ]
    );

    expect(groups).toHaveLength(3);
    expect(groups.find((group) => group.projectId === 'release')).toMatchObject({
      path: 'project:release',
      label: 'Public release',
    });
    expect(
      groups.find((group) => group.projectId === 'release')?.sessions.map((session) => session.id)
    ).toEqual(['a', 'b']);
    expect(
      groups.find((group) => group.path === '/tmp/api')?.sessions.map((session) => session.id)
    ).toEqual(['d']);
  });

  it('keeps an explicit project readable while its definition is loading', () => {
    const groups = groupSessionsByProject([
      makeSession({ projectId: 'website-launch', workingDir: '/tmp/web' }),
    ]);
