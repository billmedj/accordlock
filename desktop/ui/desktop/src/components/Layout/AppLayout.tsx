...
const AppLayoutContent: React.FC<AppLayoutContentProps> = ({ activeSessions }) => {
  ...
  return (
    <div className="flex flex-1 w-full h-full relative animate-fade-in bg-background-primary flex-row">
      ...
      {/* The sidebar is part of the application shell, not a card floating
          inside another card. Keeping this boundary flat gives the task
          canvas the visual priority. */}
      <div className="flex flex-1 w-full h-full min-h-0 flex-row">
        <motion.div
          ...
          style={{ height: '100%' }}
          className="relative h-full flex-shrink-0 overflow-hidden border-r border-border-primary bg-background-secondary"
        >
          ...
          {isNavExpanded && (
            <div
              className="absolute right-0 top-0 h-full w-1 cursor-col-resize transition-colors hover:bg-border-primary/50"
              onMouseDown={handleResizeMouseDown}
            />
          )}
        </motion.div>

        <div className="min-h-0 flex-1 overflow-hidden bg-background-primary">
          <Outlet />
