...
const i18n = defineMessages({
  ...
  createSchedule: { id: 'schedulesView.createSchedule', defaultMessage: 'New schedule' },
  description: {
    id: 'schedulesView.description',
    defaultMessage: 'Run playbooks on a schedule.',
  },
  ...
  confirmDelete: {
    ...
    defaultMessage: 'Remove schedule "{id}"? The playbook will remain.',
  },
  ...
  scheduleUnpaused: { id: 'schedulesView.scheduleUnpaused', defaultMessage: 'Schedule resumed' },
  scheduleUnpausedMsg: {
    id: 'schedulesView.scheduleUnpausedMsg',
    defaultMessage: 'Resumed "{id}"',
  },
...
const SchedulesView: React.FC<SchedulesViewProps> = ({ onClose: _onClose }) => {
  ...
  const fetchSchedules = async () => {
    ...
    try {
      ...
      setApiError('AccordLock could not load schedules. Try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleModalSubmit = async (payload: NewSchedulePayload | string) => {
    ...
    try {
      ...
      const errorMsg = 'AccordLock could not save this schedule. Check the details and try again.';
  ...
  const handleDeleteSchedule = async (id: string) => {
    ...
    try {
      ...
      const errorMsg = 'AccordLock could not delete this schedule. Try again.';
  ...
  const handlePauseSchedule = async (id: string) => {
    ...
    try {
      ...
      const errorMsg = 'AccordLock could not pause this schedule. Try again.';
      setApiError(errorMsg);
      toastError({
  ...
  const handleUnpauseSchedule = async (id: string) => {
    ...
    try {
      ...
      const errorMsg = 'AccordLock could not resume this schedule. Try again.';
      setApiError(errorMsg);
      toastError({
  ...
  const handleKillRunningJob = async (id: string) => {
    ...
    try {
      ...
      const errorMsg = 'AccordLock could not stop this job. Try again.';
      setApiError(errorMsg);
      toastError({
  ...
  const handleInspectRunningJob = async (id: string) => {
    ...
    try {
      ...
      const errorMsg = 'AccordLock could not inspect this job. Try again.';
      setApiError(errorMsg);
      toastError({
