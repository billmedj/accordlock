...
describe('ScheduleModal', () => {
  it('preserves the form when the recipe picker is cancelled', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Choose YAML file…' }));
  ...
  it('clears a validation error from create mode when reopened to edit a schedule', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Choose a valid playbook source.')).toBeInTheDocument();
    });
    ...
    expect(screen.queryByText('Choose a valid playbook source.')).not.toBeInTheDocument();
  });
});
