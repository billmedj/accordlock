...
const i18n = defineMessages({
  ...
  recipeSource: { id: 'scheduleDetailView.recipeSource', defaultMessage: 'Playbook:' },
  lastRun: { id: 'scheduleDetailView.lastRun', defaultMessage: 'Last Run:' },
  currentSession: { id: 'scheduleDetailView.currentSession', defaultMessage: 'Current task:' },
  ...
  cannotModifyRunning: {
    ...
    defaultMessage: 'This schedule is already running.',
  },
  pausedWarning: {
  ...
  recentSessions: { id: 'scheduleDetailView.recentSessions', defaultMessage: 'Recent runs' },
  loadingSessions: {
...
const ScheduleDetailView: React.FC<ScheduleDetailViewProps> = ({ scheduleId, onNavigateBack }) => {
  ...
  const fetchSessions = async (sId: string) => {
    ...
    try {
      ...
      setSessionsError('AccordLock could not load recent runs. Try again.');
    } finally {
  ...
  const fetchSchedule = useCallback(
    async (sId: string) => {
      ...
      try {
        ...
        setScheduleError('AccordLock could not load this schedule. Try again.');
      } finally {
  ...
  const handleRunNow = async () => {
    ...
    try {
      ...
      toastError({
        ...
        msg: 'AccordLock could not start this schedule. Try again.',
      });
    } finally {
  ...
  const handlePauseToggle = async () => {
    ...
    try {
      ...
      console.error('Failed to change schedule pause state:', err);
      toastError({
        title: intl.formatMessage(i18n.pauseUnpauseError),
        msg: 'AccordLock could not change this schedule. Try again.',
      });
    } finally {
      setIsActionLoading(false);
    }
  ...
  const handleKill = async () => {
    ...
    try {
      ...
      console.error('Failed to stop scheduled job:', err);
      toastError({
        title: intl.formatMessage(i18n.killJobError),
        msg: 'AccordLock could not stop this job. Try again.',
      });
    } finally {
      setIsActionLoading(false);
    }
  ...
  const handleInspect = async () => {
    ...
    try {
      ...
      console.error('Failed to inspect scheduled job:', err);
      toastError({
        title: intl.formatMessage(i18n.inspectJobError),
        msg: 'AccordLock could not inspect this job. Try again.',
      });
    } finally {
      setIsActionLoading(false);
    }
  ...
  const handleModalSubmit = async (payload: NewSchedulePayload | string) => {
    ...
    try {
      ...
      toastError({
        ...
        msg: 'AccordLock could not update this schedule. Check the details and try again.',
      });
    } finally {
      setIsActionLoading(false);
    }
