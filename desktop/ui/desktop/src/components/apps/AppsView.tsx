...
const i18n = defineMessages({
  ...
  description: {
    ...
    defaultMessage: 'Applications from your MCP servers and apps built during AccordLock tasks.',
  },
  loading: {
    id: 'appsView.loading',
  ...
  noAppsDescription: {
    ...
    defaultMessage:
      'Start a task and describe the app you need, or import an app that someone shared with you.',
  },
  customApp: {
...
export default function AppsView() {
  ...
  const refreshAppsExtensionList = useCallback(async (activeSessionId?: string) => {
    ...
    try {
      ...
      if (cacheApps.length === 0) {
        setError('AccordLock could not load apps. Try again.');
      }
    }
  }, []);
  ...
  const handleDeleteApp = async (app: GooseApp) => {
    ...
    try {
      ...
      setError('AccordLock could not delete this app. Try again.');
    } finally {
  ...
  const handleDownloadApp = async (app: GooseApp) => {
    try {
      ...
      setError('AccordLock could not export this app. Try again.');
    }
  };
  ...
  const handleUploadApp = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setError('AccordLock could not import this app. Check the file and try again.');
    }
    event.target.value = '';
  };
