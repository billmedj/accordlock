...
const i18n = defineMessages({
  ...
  disableAutoDownloadDesc: {
    ...
    defaultMessage:
      'When enabled, AccordLock will notify you of new versions without downloading them automatically.',
  },
  ...
  autoDownload: {
    ...
    defaultMessage:
      'AccordLock will download the update in the background and install it the next time you quit or restart.',
  },
  manualInstallNote: {
  ...
  readyInstallAuto: {
    ...
    defaultMessage:
      "✓ Update is ready. Restart AccordLock to finish installing it, or quit when you're done.",
  },
  installNowHint: {
...
export default function UpdateSection() {
  ...
  useEffect(() => {
    ...
    window.electron.onUpdaterEvent((event) => {
      switch (event.event) {
        ...
        case 'error':
          console.error('Updater reported an error:', event.data);
          setUpdateStatus('error');
          setUpdateInfo((prev) => ({
            ...prev,
            error: 'AccordLock could not check for updates. Try again later.',
          }));
  ...
  const downloadUpdate = async () => {
    ...
    try {
      ...
      setUpdateInfo((prev) => ({
        ...prev,
        error: 'AccordLock could not download the update. Try again later.',
      }));
      setUpdateStatus('error');
      setTimeout(() => setUpdateStatus('idle'), 5000);
    }
