...
import { isSupportedDeepLinkProtocol } from '../../../utils/deepLinks';
...
export async function addExtensionFromDeepLink(
  ...
  if (!isSupportedDeepLinkProtocol(parsedUrl.protocol)) {
    toastService.handleError(
      'Invalid Protocol',
      'Failed to install extension: URL must use the accordlock:// scheme',
      { shouldThrow: true }
    );
  }
