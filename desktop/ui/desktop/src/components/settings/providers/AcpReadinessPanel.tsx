...
const i18n = defineMessages({
  adapterFound: {
    ...
    defaultMessage: 'Provider available',
  },
  adapterNotFound: {
    id: 'acpReadinessPanel.adapterNotFound',
    defaultMessage: 'Provider setup required',
  },
  connected: {
  ...
  connectionNotChecked: {
    ...
    defaultMessage: 'Check the connection to continue.',
  },
  authenticationHelp: {
    id: 'acpReadinessPanel.authenticationHelp',
    defaultMessage: 'Sign in to your provider, then check again.',
  },
  checkAgain: {
  ...
  connectionFailed: {
    id: 'acpReadinessPanel.connectionFailed',
    defaultMessage: 'Could not verify this provider. Check its setup and try again.',
  },
  refreshFailed: {
    id: 'acpReadinessPanel.refreshFailed',
    defaultMessage: 'Could not refresh provider status.',
  },
});
...
export default function AcpReadinessPanel({
  ...
  const check = useCallback(async () => {
    ...
    try {
      ...
      if (!wasAborted(error)) {
        ...
        onError(intl.formatMessage(i18n.connectionFailed));
      }
    } finally {
      if (!controller.signal.aborted) setIsChecking(false);
    }
  }, [intl, onError, provider.name, startRequest]);
  ...
  const configure = async () => {
    ...
    try {
      ...
      if (!wasAborted(error)) {
        console.error('Provider enablement failed:', error);
        onError(intl.formatMessage(i18n.connectionFailed));
      }
    } finally {
      if (!controller.signal.aborted) setIsChecking(false);
      ...
      <div className="rounded-md border border-border-primary p-3 space-y-2">
        ...
        {readinessError && (
          <div className="space-y-1 text-sm text-red-600 break-words">
            <div>{intl.formatMessage(i18n.connectionFailed)}</div>
