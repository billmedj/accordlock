...
const i18n = defineMessages({
  ...
  catalogLoadFailed: {
    id: 'providerCatalogPicker.catalogLoadFailed',
    defaultMessage:
      "AccordLock couldn't load the provider catalog. Check your connection and try again.",
  },
  providerLoadFailed: {
    id: 'providerCatalogPicker.providerLoadFailed',
    defaultMessage: "AccordLock couldn't load this provider. Try again or choose another provider.",
  },
  noProvidersFound: {
...
export default function ProviderCatalogPicker({
  ...
  const fetchProviders = useCallback(
    async (format: string) => {
      ...
      try {
        ...
        console.error('[ProviderCatalogPicker] Failed to load provider catalog:', err);
        setError(intl.formatMessage(i18n.catalogLoadFailed));
      } finally {
        setLoading(false);
      }
    },
  ...
  useEffect(() => {
    void fetchProviders(selectedFormat);
  ...
  const handleProviderSelect = async (providerId: string) => {
    ...
    try {
      ...
      console.error('[ProviderCatalogPicker] Failed to load provider template:', err);
      setError(intl.formatMessage(i18n.providerLoadFailed));
    } finally {
      setLoading(false);
    }
  };
