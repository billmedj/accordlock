...
export default function ModelsBottomBar({
  ...
  const [displayModelName, setDisplayModelName] = useState<string>(() =>
    currentModel ? getModelDisplayName(currentModel) : intl.formatMessage(i18n.selectModel)
  );
  ...
  const displayModel = currentModel
    ...
    : providerDefaultModel
      ? getModelDisplayName(providerDefaultModel)
      : displayModelName;
  ...
  return (
    <div className="relative flex min-w-0 max-w-full items-center" ref={dropdownRef}>
      <DropdownMenu>
        <DropdownMenuTrigger
          data-testid="model-selector-trigger"
          title={triggerLabel}
          aria-label={`${intl.formatMessage(i18n.currentModel)}: ${triggerLabel}`}
          className="flex h-8 w-full min-w-0 max-w-[180px] items-center overflow-hidden rounded-lg bg-background-secondary/55 px-2 text-text-primary/75 transition-colors hover:cursor-pointer hover:bg-background-tertiary hover:text-text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-border-secondary md:max-w-[200px] lg:max-w-[280px]"
        >
