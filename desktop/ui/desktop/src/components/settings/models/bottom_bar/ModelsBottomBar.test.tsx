...
describe('ModelsBottomBar', () => {
  ...
  it('shows the active session model once the session has loaded', async () => {
    ...
    expect(screen.getAllByText('Display session-model')).not.toHaveLength(0);
  ...
  it('shows the configured model when there is no active session', async () => {
    ...
    expect(screen.getAllByText('Display config-model')).not.toHaveLength(0);
    expect(screen.queryByTestId('model-loading-state')).not.toBeInTheDocument();
  });

  it('uses a compact 32px chip target', () => {
    renderWithIntl(
      <ModelsBottomBar
        sessionId={null}
        dropdownRef={createDropdownRef()}
    ...
    expect(screen.getByTestId('model-selector-trigger')).toHaveClass('h-8', 'rounded-lg', 'px-2');
  });
});
