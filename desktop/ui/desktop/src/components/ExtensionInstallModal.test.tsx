...
describe('ExtensionInstallModal', () => {
  ...
  describe('Extension Request Handling', () => {
    it('should handle trusted extension (default behaviour, no allowlist)', async () => {
      ...
      await act(async () => {
        await eventHandler({}, 'accordlock://extension?cmd=npx&arg=test-extension&name=TestExt');
      });
    ...
    it('should handle trusted extension (from allowlist)', async () => {
      ...
      await act(async () => {
        await eventHandler({}, 'accordlock://extension?cmd=npx&arg=test-extension&name=AllowedExt');
      });
    ...
    it('should handle warning mode', async () => {
      ...
      await act(async () => {
        await eventHandler(
          {},
          'accordlock://extension?cmd=npx&arg=untrusted-extension&name=UntrustedExt'
        );
      });
    ...
    it('should handle blocked extension', async () => {
      ...
      await act(async () => {
        await eventHandler(
          {},
          'accordlock://extension?cmd=npx&arg=blocked-extension&name=BlockedExt'
        );
      });
  ...
  describe('Modal Actions', () => {
    it('should dismiss modal correctly', async () => {
      ...
      await act(async () => {
        await eventHandler({}, 'accordlock://extension?cmd=npx&arg=test&name=Test');
      });
    ...
    it('should handle successful extension installation', async () => {
      ...
      expect(addExtensionFromDeepLink).toHaveBeenCalledWith(
        'accordlock://extension?cmd=npx&arg=test&name=Test',
        mockAddExtension,
        expect.any(Function)
      );
    ...
    it('shows a fixed installation error without exposing internal details', async () => {
      const privateDetail =
        'HTTP 500 from https://private-extension.example?token=secret payload={key:secret}';
      const consoleError = vi.spyOn(console, 'error').mockImplementation(() => undefined);
      vi.mocked(addExtensionFromDeepLink).mockRejectedValueOnce(new Error(privateDetail));
      mockElectron.getAllowedExtensions.mockResolvedValue([]);
      ...
      expect(await screen.findByRole('alert')).toHaveTextContent(
        'AccordLock could not install this extension. Check its configuration and try again.'
      );
