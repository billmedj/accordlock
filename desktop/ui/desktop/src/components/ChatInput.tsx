...
import { ArrowUp, Bug, LoaderCircle, ScrollText } from 'lucide-react';
...
const i18n = defineMessages({
  ...
  restartingSession: {
    ...
    defaultMessage: 'Restarting task…',
  },
  typeMessage: {
...
export function shouldQueueChatInputDuringLoading(
  landingMode: boolean,
  isLoading: boolean,
  hasSubmittableContent: boolean
): boolean {
  return !landingMode && isLoading && hasSubmittableContent;
}
...
export default function ChatInput({
  ...
  const handleInterruptionAndQueue = () => {
    if (
      !shouldQueueChatInputDuringLoading(landingMode, isLoading, Boolean(hasSubmittableContent))
    ) {
      return false;
    }
  ...
  const onFormSubmit = (e: React.FormEvent | React.MouseEvent) => {
    ...
    if (isLoading) {
      if (!landingMode && hasSubmittableContent) {
  ...
  const isSubmitButtonDisabled =
    ...
    isTranscribing ||
    queueProcessingBlocked ||
    (landingMode && isLoading) ||
    chatState === ChatState.RestartingAgent;

  const isLandingStartPending = landingMode && isLoading;
  const effectiveSubmitLabel = isLandingStartPending
  ...
  const getSubmitButtonTooltip = (): string => {
    ...
    if (isLandingStartPending) return intl.formatMessage(i18n.starting);
    ...
    return effectiveSubmitLabel ?? intl.formatMessage(i18n.send);
  };
  ...
  return (
    ...
    >
      ...
      <form onSubmit={onFormSubmit} className="relative">
        <div className="relative">
          <textarea
            ...
            autoFocus
            id="dynamic-textarea"
            placeholder={isRecording ? '' : (placeholder ?? getNavigationShortcutText(intl))}
            value={displayValue}
            ...
            ref={textAreaRef}
            rows={1}
            readOnly={isRecording || isLandingStartPending}
            style={{
            ...
            className={cn(
              'w-full resize-none border-none bg-transparent px-3 pt-3 pb-1.5 text-text-primary outline-none placeholder:text-text-secondary focus:ring-0',
              landingMode ? 'text-base leading-6' : 'text-sm'
            )}
          />
      ...
      <div ref={bottomBarRef} className="relative flex flex-row items-center gap-2 px-3 py-2">
        <div
          className={cn(
            landingMode ? 'flex min-w-0 flex-1 items-center gap-1.5 overflow-hidden' : 'contents'
          )}
        >
          {/* Left: model selector */}
          <Tooltip>
            <div className={cn(landingMode && 'min-w-0 flex-[0_1_45%] overflow-hidden')}>
              <ModelsBottomBar
                sessionId={sessionId}
          ...
          {/* Keep both task-defining choices visible on the landing screen. */}
          {(landingMode || !isBottomBarNarrow) && (
            <DirSwitcher
              className={landingMode ? 'min-w-0 flex-1' : ''}
              workingDir={currentWorkingDir}
            />
          )}

          {showSessionStatus && !isBottomBarNarrow && currentWorkingDir && (
            <GitBranchIndicator dir={currentWorkingDir} className="ml-1" />
          )}
        </div>

        {/* Spacer */}
        <div className={cn('flex-1', landingMode && 'hidden')} />

        {!isBottomBarNarrow && (
          <>
            {/* Right: cost tracker (when enabled) */}
            {showSessionStatus && COST_TRACKING_ENABLED && (
              <CostTracker
        ...
        {dictationProvider && (!landingMode || !isBottomBarNarrow) && (
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                type="button"
        ...
        {isLoading && !landingMode && !hasSubmittableContent ? (
          <Button
            type="button"
            onClick={handleStop}
            size="sm"
            ...
            <TooltipTrigger asChild>
              <span className="shrink-0">
                <Button
                  type="button"
                  size="sm"
                  shape={effectiveSubmitLabel ? 'pill' : 'round'}
                  variant={effectiveSubmitLabel ? 'default' : 'ghost'}
                  disabled={isSubmitButtonDisabled}
                  aria-label={effectiveSubmitLabel ?? intl.formatMessage(i18n.send)}
                  aria-busy={isLandingStartPending}
                  onClick={onFormSubmit}
                  className={cn(
                    effectiveSubmitLabel ? 'px-3' : 'bg-background-tertiary',
                ...
                >
                  {effectiveSubmitLabel ? <span>{effectiveSubmitLabel}</span> : null}
                  {isLandingStartPending ? (
