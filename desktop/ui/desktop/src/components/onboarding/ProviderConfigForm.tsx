...
export default function ProviderConfigForm({ provider, onConfigured }: ProviderConfigFormProps) {
  ...
  const handleSetupError = (cause: unknown) => {
    console.error('[ProviderConfigForm] Provider setup failed:', cause);
    setError(intl.formatMessage(i18n.setupFailed));
  };
  ...
  const renderForm = () => {
    ...
    if (isOAuthProvider) {
      return (
        <OAuthForm provider={provider} onConfigured={onConfigured} onError={handleSetupError} />
      );
    }
    return (
      <ApiKeyForm provider={provider} onConfigured={onConfigured} onError={handleSetupError} />
    );
  };
