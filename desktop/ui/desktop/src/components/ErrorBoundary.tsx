...
const i18n = defineMessages({
  heading: {
    ...
    defaultMessage: 'AccordLock needs to restart',
  },
  errorWithVersion: {
    id: 'errorBoundary.errorWithVersion',
    defaultMessage: 'An error occurred in AccordLock v{version}.',
  },
  errorGeneric: {
  ...
  recovery: {
    id: 'errorBoundary.recovery',
    defaultMessage: 'Your work is saved. Reload the app to continue.',
  },
  reload: {
...
export function ErrorUI({ error: _error }: { error?: string } = {}) {
  void _error;
  const intl = useIntl();
  const handleReload = () => {
    trackEvent({
  ...
  return (
    <div className="fixed inset-0 w-full h-full flex flex-col items-center justify-center gap-6 bg-background">
      <div className="flex flex-col items-center gap-4 max-w-[600px] text-center px-6">
        ...
        <p className="text-sm text-text-secondary dark:text-muted-foreground">
          {intl.formatMessage(i18n.recovery)}
        </p>
