...
export const GitBranchIndicator: React.FC<{ dir: string; className?: string }> = ({
  ...
  useEffect(() => {
    ...
    setBranches([]);
    setSearch('');
    Promise.all([window.electron.getGitBranchInfo(), window.electron.listGitBranches()])
      .then(([info, list]) => {
        if (!cancelled) {
  ...
  const handleSwitch = async (target: string) => {
    ...
    const result = await window.electron
      .switchGitBranch(target)
      .catch((err: Error) => ({ success: false, canceled: false, error: err.message }));
    if (result.success) {
      setBranch(target);
