...
describe('ParameterInputModal', () => {
  ...
  describe('Rendering', () => {
    it('renders modal with parameters', () => {
      ...
      expect(screen.getByText('Playbook inputs')).toBeInTheDocument();
  ...
  describe('Form Submission', () => {
    it('calls onSubmit with parameter values when submitted', async () => {
      ...
      const submitButton = screen.getByText('Run playbook');
    ...
    it.each([
      ...
      async ({ parameter, initialValue, visibleValue }) => {
        ...
        await user.click(screen.getByText('Run playbook'));
  ...
  describe('Cancel Behavior', () => {
    it('shows cancel options when cancel is clicked and parameters exist', async () => {
      ...
      expect(screen.getByText('Cancel playbook setup')).toBeInTheDocument();
    ...
    it('calls onClose when starting without the playbook', async () => {
      const user = userEvent.setup();
      renderWithIntl(<ParameterInputModal {...defaultProps} />);

      await user.click(screen.getByText('Cancel'));
      await user.click(screen.getByText('Start without playbook'));

      expect(defaultProps.onClose).toHaveBeenCalledTimes(1);
    });

    it('returns to the setup form', async () => {
      const user = userEvent.setup();
      renderWithIntl(<ParameterInputModal {...defaultProps} />);
