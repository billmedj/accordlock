...
const i18n = defineMessages({
  ...
  duplicated: {
    ...
    defaultMessage: 'Task duplicated',
  },
  duplicateFailed: {
  ...
  jsonFailed: {
    ...
    defaultMessage: 'Could not load audit trail: {error}',
  },
...
export default function SessionActionsHeader({
  ...
  const handleViewJson = useCallback(async () => {
    ...
    try {
      ...
      if (jsonLoadRequestId.current !== loadRequestId) return;
      console.error('Failed to load task record:', error);
      setIsJsonOpen(false);
      toast.error(
  ...
  const handleViewModelInteractions = useCallback(async () => {
    ...
    try {
      ...
      if (jsonLoadRequestId.current !== loadRequestId) return;
      console.error('Failed to load model interactions:', error);
      setIsJsonOpen(false);
      toast.error(
