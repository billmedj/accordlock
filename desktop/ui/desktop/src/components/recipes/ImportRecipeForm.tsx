...
const i18n = defineMessages({
  importRecipeTitle: {
    ...
    defaultMessage: 'Import playbook',
  },
  recipeDeeplinkLabel: {
  ...
  deeplinkPlaceholder: {
    ...
    defaultMessage: 'Paste an accordlock://recipe?config=... link',
  },
  deeplinkHint: {
    id: 'importRecipeForm.deeplinkHint',
    defaultMessage: 'Links begin with "accordlock://recipe?config="',
  },
  or: {
  ...
  recipeFileLabel: {
    ...
    defaultMessage: 'Playbook file',
  },
  recipeFileHint: {
    id: 'importRecipeForm.recipeFileHint',
    defaultMessage: 'Choose a YAML or JSON playbook',
  },
  example: {
  ...
  reviewWarning: {
    ...
    defaultMessage: 'Review imported playbooks before running them.',
  },
  cancel: {
  ...
  schemaDescription: {
    ...
    defaultMessage: 'Required: title, description, and instructions or prompt.',
  },
});
...
const importRecipeSchema = z
  .object({
    deeplink: z
      .string()
      .refine(
        (value) => !value || isSupportedDeepLink(value.trim(), 'recipe?config='),
        'Use an accordlock://recipe?config=... link'
      ),
    recipeUploadFile: z
      .instanceof(File)
      .nullable()
  ...
  .refine((data) => (data.deeplink && data.deeplink.trim()) || data.recipeUploadFile, {
    message: 'Add a share link or playbook file',
    path: ['deeplink'],
  });
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        if (value.deeplink && value.deeplink.trim()) {
          ...
          if (!parsedRecipe) {
            throw new Error('Invalid playbook link');
          }
          recipe = parsedRecipe;
        } else {
        ...
        toastSuccess({
          ...
          msg: 'Playbook imported',
        });
      } catch (error) {
        console.error('Failed to import recipe:', error);

        toastError({
          title: 'Could not import playbook',
          msg: 'AccordLock could not import this playbook. Check the source and try again.',
        });
      } finally {
        setImporting(false);
      }
  ...
  const handleDeeplinkChange = async (
    ...
    if (value.trim()) {
      try {
        ...
        console.error('Invalid playbook deeplink:', error);
        toastError({
  ...
  const handleRecipeUploadChange = async (file: File | undefined) => {
    ...
    if (file) {
      try {
        ...
        console.error('Invalid playbook file:', error);
        toastError({
          title: 'Invalid playbook file',
          msg: 'Check the file and try again.',
        });
      }
    }
  };
