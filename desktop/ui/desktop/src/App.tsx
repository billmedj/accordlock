...
import { isSupportedDeepLink } from './utils/deepLinks';
import { ErrorUI } from './components/ErrorBoundary';
import { ExtensionInstallModal } from './components/ExtensionInstallModal';
import { ConfiguredTaskAuthorizationController } from './components/accordlock/TaskAuthorizationController';
import { ApprovalInboxController } from './components/accordlock/ApprovalInboxController';
import { ApprovalCenterRoute } from './components/accordlock/ApprovalCenterRoute';
import GlobalAuditView from './components/accordlock/GlobalAuditView';
          ...
          window.dispatchEvent(
            new CustomEvent(AppEvents.ADD_ACTIVE_SESSION, {
              detail: {
                ...
                noAutoSubmit,
                deferTaskReview: Boolean(newSession.recipe),
              },
            })
          );
  ...
  return (
    <GlobalAuditView
      onOpenTask={(sessionId) =>
        setView('pair', { disableAnimation: true, resumeSessionId: sessionId })
      }
    />
  );
};
...
export function AppInner() {
  ...
  useEffect(() => {
    try {
      ...
      setFatalError('AccordLock could not finish starting. Reload the app to try again.');
    }
  }, []);
    ...
    acpListSessions()
      .then(({ sessions }) => {
        ...
        for (const s of phantom) {
          void revokeBeforeAccordLockSessionDeletion(s.id, () => acpDeleteSession(s.id))
            .then(() => {
              window.dispatchEvent(
                new CustomEvent(AppEvents.SESSION_DELETED, { detail: { sessionId: s.id } })
              );
            })
            .catch((error) => {
              console.error('Could not safely clean up an empty AccordLock session:', error);
            });
        }
      })
      .catch(() => {});
  ...
  useEffect(() => {
    const handleOpenSharedSession = async (_event: IpcRendererEvent, ...args: unknown[]) => {
      ...
      if (!isSupportedDeepLink(link, 'sessions/nostr')) {
      ...
      try {
        ...
        toast.error('AccordLock could not import this task. Check the link and try again.');
        navigate('/sessions');
      } finally {
        if (nostrImportInFlight.current === link) {
  ...
  useEffect(() => {
    const handleFatalError = (_event: IpcRendererEvent, ...args: unknown[]) => {
      const diagnostic = args[0] as string;
      console.error('Encountered a fatal error:', diagnostic);
      setFatalError('AccordLock stopped unexpectedly. Reload the app to continue.');
    };
  ...
  useEffect(() => {
    const handleNotificationOpen = (_event: IpcRendererEvent, ...args: unknown[]) => {
      try {
        navigate(routeForNotificationTarget(parseNotificationOpenTarget(args[0])));
      } catch {
        // The main process validates this target too. A malformed renderer
        // event is ignored and never changes task authority.
      }
    };

    window.electron.on('accordlock:notification-open', handleNotificationOpen);
    return () => window.electron.off('accordlock:notification-open', handleNotificationOpen);
  }, [navigate]);
  ...
  if (fatalError) {
    return <ErrorUI error={fatalError} />;
  }
  ...
  return (
    <>
      ...
      <ConfiguredTaskAuthorizationController />
        ...
        <div style={{ position: 'relative', width: '100%', height: '100%' }}>
          <Routes>
            ...
            >
              ...
              <Route path="approvals" element={<ApprovalCenterRoute />} />
