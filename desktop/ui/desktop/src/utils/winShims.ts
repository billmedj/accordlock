...
/**
 * Ensures Windows shims are available in %LOCALAPPDATA%\AccordLock\bin.
 * This allows bundled helper executables to be found without changing the user's system PATH.
 */
export async function ensureWinShims(): Promise<void> {
  ...
  try {
    if (!(await fs.promises.stat(srcDir)).isDirectory()) return;
  } catch {
    return;
  }
  ...
  try {
    ...
    if (!currentPath.toLowerCase().includes(tgtDir.toLowerCase())) {
      ...
      log.info(`Added ${tgtDir} to PATH for AccordLock processes only`);
    } else {
