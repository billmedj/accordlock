    ...
    if (isUsingGitHubFallback) {
      ...
      try {
        ...
        const dialogResult = (await dialog.showMessageBox({
          ...
          detail: `The update has been downloaded and extracted. To complete the installation:\n\n1. Click "Open Folder" to view the new AccordLock.app\n2. Quit AccordLock (this app will close)\n3. Drag the new AccordLock.app to your Applications folder\n4. Replace the existing app when prompted\n\nThe update will be available the next time you launch AccordLock.`,
...
export function setupAutoUpdater(tray?: Tray) {
  ...
  if (!owner || !repo) {
    throw new Error(
      'AccordLock updates are disabled because no signed release repository is configured.'
    );
  }

  // A release build must opt into an AccordLock-owned update feed at build time. Never
  // inherit Goose's public feed: an updater is executable supply-chain authority.
  const feedConfig = {
  ...
  autoUpdater.on('update-downloaded', (info: UpdateInfo) => {
    ...
    const notification = new Notification({
      title: 'Update Ready',
      body: `Version ${info.version} will be installed when you quit AccordLock. Click to install now.`,
    });
    notification.show();
...
function updateTrayIcon(hasUpdate: boolean) {
  ...
  const usesTemplateIcon = process.platform === 'darwin';
  const iconName = usesTemplateIcon
    ? hasUpdate
  ...
  trayRef.setToolTip(hasUpdate ? 'AccordLock - Update Available' : 'AccordLock');
