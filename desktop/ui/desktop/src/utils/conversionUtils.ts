...
export const GENERIC_USER_FACING_ERROR = 'Something went wrong. Please try again.';

/**
 * Return stable UI copy without reflecting provider-controlled error details.
 * Use `formatErrorForLogging` when diagnostics are required in internal logs.
 */
export function userFacingErrorMessage(
  _error: unknown,
  fallback: string = GENERIC_USER_FACING_ERROR
): string {
  const message = fallback.trim();
  return message || GENERIC_USER_FACING_ERROR;
}
