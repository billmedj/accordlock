...
export class GitHubUpdater {
  ...
  private readonly repo = process.env.GITHUB_REPO?.trim();
  private readonly bundleName = process.env.GOOSE_BUNDLE_NAME || 'AccordLock';
  ...
  async checkForUpdates(): Promise<UpdateCheckResult> {
    ...
    try {
      if (!this.apiUrl) {
        throw new Error('AccordLock update repository is not configured');
      }
      ...
      const response = await fetch(this.apiUrl, {
        headers: {
          ...
          'User-Agent': `AccordLock-Desktop/${app.getVersion()}`,
        },
