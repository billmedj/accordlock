...
import { isSupportedDeepLink, PRIMARY_DEEP_LINK_SCHEME } from '../utils/deepLinks';
...
export async function generateDeepLink(recipe: Recipe): Promise<string> {
  ...
  return `${PRIMARY_DEEP_LINK_SCHEME}://recipe?config=${encoded}`;
}
...
export async function parseDeeplink(deeplink: string): Promise<Recipe | null> {
  try {
    ...
    if (!isSupportedDeepLink(cleanLink, 'recipe?config=')) {
      throw new Error(
        `Invalid deeplink format. Expected: ${PRIMARY_DEEP_LINK_SCHEME}://recipe?config=...`
      );
    }

    const recipeEncoded = cleanLink.slice(cleanLink.indexOf('=') + 1);
