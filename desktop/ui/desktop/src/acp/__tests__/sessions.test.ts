...
describe('ACP sessions', () => {
  ...
  it('carries the recipe parameter scope and project in new-session metadata', async () => {
  ...
  it('archives and restores a task without deleting its session', async () => {
    const client = {
      goose: {
    ...
    expect(client.goose.sessionArchive_unstable).toHaveBeenCalledWith({ sessionId: 'session-1' });
