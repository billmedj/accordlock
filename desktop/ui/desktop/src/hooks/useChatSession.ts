  ...
  const onFinish = useCallback(
    async (error?: string): Promise<void> => {
      if (error) {
        ...
        const safeError = userFacingErrorMessage(
          error,
          "AccordLock couldn't complete this request. Try again."
        );
        setLastRunError(safeError);
        toastError({ title: "Couldn't send message", msg: safeError });
      } else {
    ...
    async (
      ...
      try {
        ...
        toastError({
          ...
          msg: userFacingErrorMessage(error, "AccordLock couldn't update this message. Try again."),
        });
      }
    },
    [getCurrentSnapshot, onFinish, sessionId]
