...
export function prependUnique(
  ...
  if (session.archivedAt) return prev;
...
function mergeWithEmptyLocals(
  ...
  const activeListed = listed.filter((session) => !session.archivedAt);
  const emptyLocals = prev.filter(
    (local) => local.messageCount === 0 && !activeListed.some((s) => s.id === local.id)
  );
  return [...emptyLocals, ...activeListed].slice(0, MAX_RECENT_SESSIONS);
}
...
export function useNavigationSessions() {
  ...
  const recentSessionsByProject = useMemo(
    () => groupSessionsByProject(recentSessions, projects),
  ...
  const fetchSessions = useCallback(async () => {
    try {
      ...
      setRecentSessions(sessions.filter((session) => !session.archivedAt));
    } catch (error) {
  ...
  const fetchProjects = useCallback(async () => {
    try {
      ...
      // Recent tasks remain available with workspace labels if projects cannot load.
    ...
    const handleSessionDeleted = (event: Event) => {
      ...
      acpListRecentSessions(MAX_RECENT_SESSIONS)
        .then((sessions) => {
          ...
          setRecentSessions(
            sessions.filter((session) => session.id !== sessionId && !session.archivedAt)
          );
        })
