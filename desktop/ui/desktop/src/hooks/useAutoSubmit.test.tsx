...
describe('useAutoSubmit', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    clearAccordLockTaskAuthorization('sess-1');
  });
  ...
  it('keeps the prompt locked for PENDING and REJECTED tasks, then submits once APPROVED', () => {
    const handleSubmit = vi.fn();
    const dispatchEventSpy = vi.spyOn(window, 'dispatchEvent');
    const wrapper = ({ children }: PropsWithChildren) => (
      <MemoryRouter initialEntries={['/pair?resumeSessionId=sess-1']}>{children}</MemoryRouter>
    ...
    renderHook(
      () => {
        const taskApproved = useAccordLockTaskAuthorized('sess-1');
    ...
    act(() => setAccordLockTaskAuthorization('sess-1', 'REJECTED'));
    expect(handleSubmit).not.toHaveBeenCalled();
    expect(dispatchEventSpy).not.toHaveBeenCalled();

    act(() =>
      setAccordLockTaskAuthorization('sess-1', 'APPROVED', Math.floor(Date.now() / 1_000) + 60)
    );
    expect(handleSubmit).toHaveBeenCalledTimes(1);
    expect(handleSubmit).toHaveBeenCalledWith(initialMessage);
    expect(dispatchEventSpy).toHaveBeenCalledTimes(1);
